...
def ai_animation(frametime, duration):
    anim = ["⢿", "⣻", "⣽", "⣾", "⣷", "⣯", "⣟", "⡿"]
    fr_ctr = frametime
    print(f"AI playing - ", end="")
    while fr_ctr < duration:
        for frame in anim:
            print(f"{frame}  ", end="")
            sleep(frametime)
            print("\b\b\b", end="")
...
class board:
    ...
    def hvai(self, turn):
        ...
        if turn == self.p2:
            ...
            # async_result = pool.apply_async(self.findBestMove)

            # # best_move = Thread(target=board.findBestMove(self)).start()
            best_move = board.findBestMove(self)
            # # Thread(target=ai_animation(0.025, 1)).start()

            # pool.apply_async(ai_animation, (0.025, 0.2))

            # best_move = async_result.get()
            # async_result = pool.apply_async(board.findbestmove, (self))
